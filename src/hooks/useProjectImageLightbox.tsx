...
function isLightboxBlocked(target: HTMLImageElement) {
    if (target.closest('[data-lightbox-disabled="true"]'))
        return true;
...
export function useProjectImageLightbox() {
    ...
    const projectImageLightbox = activeImage && typeof document !== 'undefined'
        ? createPortal(<div className="fixed inset-0 z-[500] flex items-center justify-center p-6 md:p-10" role="dialog" aria-modal="true" aria-label="Expanded project image" onClick={() => setActiveImage(null)}>
            <div className="absolute inset-0 bg-black/35" style={{
                backdropFilter: 'blur(8px)',
                WebkitBackdropFilter: 'blur(8px)',
            }}/>
            <img src={activeImage.src} alt={activeImage.alt} className="relative z-[1] block w-auto h-auto max-w-[92vw] max-h-[88vh] rounded-[4px] object-contain shadow-[0_24px_72px_rgba(0,0,0,0.35)]" onClick={(event) => event.stopPropagation()}/>
          </div>, document.body)
        : null;
    return {
